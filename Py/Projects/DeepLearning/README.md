...
- `class_9.py`：从零拆解 Transformer Encoder，包含多头自注意力、位置嵌入、训练曲线和注意力热力图。
...
## 第 9 课：从零拆解 Transformer Encoder

`class_9.py` 把 Transformer 的核心结构拆成一份可运行、可观察的教学脚本。它不依赖外部数据集，而是构造三条小任务：一条用 Encoder 做 `<MARK>` 标记查找分类，一条用 Decoder 做自回归 next-token prediction，另一条用完整 Encoder-Decoder 做 seq2seq copy。

### 第 9 课在讲什么

第 2 课已经把 Transformer 放进 RNN / LSTM / GRU 的序列建模对比里。第 9 课则更进一步，把 Transformer 自身拆开来看：

- token embedding 如何把离散 token id 变成连续向量。
- position embedding 如何补上序列顺序信息。
- scaled dot-product attention 如何用 `QK^T / sqrt(d)` 得到注意力分布。
- multi-head self-attention 如何让模型从多个表示子空间读取上下文。
- residual connection、LayerNorm 和 FFN 如何组成一个稳定的 Transformer Encoder Block。
- Decoder-only Transformer 如何使用 causal mask，保证当前位置不能偷看未来 token。
- 完整 Encoder-Decoder Transformer 如何用 cross-attention 从 Encoder memory 读取源序列信息。
- 如何把注意力权重导出成 heatmap，观察模型到底在看哪里。
...
标签是 `7`，也就是 `<MARK>` 后面一格的数字。由于 `<MARK>` 的位置每个样本都会变化，模型不能只记住固定位置，必须先定位 `<MARK>`，再读取它后面的 token。

这让任务足够小、训练很快，同时又能清楚展示 Transformer 的核心能力：根据上下文动态建立 token 之间的依赖关系。

脚本里还加入了一个 Decoder-only 任务：

`<CLS> 3 4 5 6 7 ...`

Decoder 的目标是自回归预测下一个 token。这里最关键的是 causal mask：第 `t` 个位置只能关注自己和左侧历史，不能关注右侧未来。这样它才是在“生成”，而不是提前看见答案。

最后还有一个完整 Encoder-Decoder 任务：

`source: 9 0 6 7 7 5`

`target: 9 0 6 7 7 5`

这个任务看起来是 copy，但 source 是随机数字，Decoder 不能只靠历史猜出下一个 token，必须通过 cross-attention 读取 Encoder 输出的 source memory。它对应的就是机器翻译里常见的“Encoder 理解源句，Decoder 生成目标句”的完整结构。

### 第 9 课运行后会生成什么

运行 `class_9.py` 后，会在 `artifacts/Class_9/` 下生成：

- `class9_transformer_marker_classifier.pt`：Transformer Encoder 分类器 checkpoint。
- `class9_token_decode_demo.txt`：token id 解码成人类可读 token 的示例说明。
- `class9_transformer_training_curves.png`：训练/验证 loss 与 accuracy 曲线。
- `class9_transformer_attention_heatmap.png`：最后一层平均多头注意力热力图。
- `class9_transformer_attention_layer1.png`、`class9_transformer_attention_layer2.png`、`class9_transformer_attention_layer3.png`：逐层平均注意力热力图。
- `class9_transformer_attention_last_layer_head1.png` 到 `class9_transformer_attention_last_layer_head4.png`：最后一层每个 head 的注意力热力图。
- `class9_transformer_marker_attention_bar.png`：单独展示 `<MARK>` 这一行注意力分布的条形图。
- `class9_transformer_ablation_comparison.png`：不同设计选择的轻量对比实验图。
- `class9_transformer_report.txt`：模型结构、最终指标和预测样例报告。
- `class9_transformer_decoder_lm.pt`：Decoder-only 语言模型 checkpoint。
- `class9_decoder_training_curves.png`：Decoder next-token prediction 训练曲线。
- `class9_decoder_causal_attention_heatmap.png`：Decoder causal self-attention 热力图。
- `class9_transformer_decoder_report.txt`：Decoder 训练结果、样本预测和 greedy 生成报告。
- `class9_transformer_seq2seq.pt`：完整 Encoder-Decoder Transformer checkpoint。
- `class9_seq2seq_training_curves.png`：Encoder-Decoder seq2seq copy 训练曲线。
- `class9_seq2seq_cross_attention_heatmap.png`：Decoder 对 Encoder memory 的 cross-attention 热力图。
- `class9_transformer_seq2seq_report.txt`：完整 Transformer 结构、copy 预测和 greedy decoding 报告。

### 第 9 课如何读输出结果

- `class9_transformer_training_curves.png`：看模型是否从随机猜测逐步学会标记查找规则。
- `class9_token_decode_demo.txt`：看模型输入的整数 token id 如何还原成 `<CLS>`、`<MARK>` 和数字 token。
- `class9_transformer_attention_heatmap.png`：横轴是被关注 token，纵轴是发出 query 的 token；蓝色边框标出 `<MARK>` 列，绿色边框标出答案 token 列。
- `class9_transformer_marker_attention_bar.png`：更直接地看 `<MARK>` 位置是否把最高权重分给答案 token。
- `class9_transformer_ablation_comparison.png`：比较 `<MARK>` 读出、`<CLS>` 读出、去掉 position embedding、少头、浅层和更难 offset 任务的差异。
- `class9_transformer_report.txt`：适合快速检查参数量、层数、头数、最终验证精度和若干预测样例。
- `class9_decoder_causal_attention_heatmap.png`：看右上角未来区域是否被 causal mask 压到接近 0。
- `class9_transformer_decoder_report.txt`：看 Decoder 是否能从 `<CLS> 3` 继续生成 `4 5 6 7 ...`。
- `class9_seq2seq_cross_attention_heatmap.png`：看 Decoder 每个位置在生成时从 Encoder source 的哪个位置读取信息。
- `class9_transformer_seq2seq_report.txt`：看完整 Encoder-Decoder 是否能在 teacher forcing 和 greedy decoding 下都完成 copy。

### 第 9 课核心收获

- Transformer 的关键不是“层很多”，而是 self-attention 能让任意位置之间直接建立联系。
- 多头注意力可以理解为从多个角度同时读取上下文。
- position embedding 是必要的，否则 attention 本身并不知道 token 的前后顺序。
- 残差连接和 LayerNorm 是让 Transformer 稳定训练的重要工程结构。
- Encoder 偏向“理解整段输入”，Decoder 偏向“按顺序生成下一个 token”。
- Decoder 的 causal mask 是自回归生成的关键，否则模型会偷看未来。
- 完整 Encoder-Decoder 比 Decoder-only 多了 cross-attention，这一步让目标序列生成过程能够读取源序列。
- 注意力热力图不能等同于完整解释，但它是观察模型行为的一个非常直观的入口。
...
6. 再看 `Class_6.py`，把注意力从“模型能做什么”进一步扩展到“模型会在哪里失效，以及我们如何分析和缓解这些失效”。
7. 最后看 `class_9.py`，把 Transformer Encoder 的内部结构、训练过程和注意力可视化拆开吃透。

---

## 一句话总结

这组脚本覆盖了从监督学习、序列建模、目标检测、表示学习、强化学习到 Transformer 结构拆解的一条入门实践路线，适合作为边学边跑的教学型代码仓库。
